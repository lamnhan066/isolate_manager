...
try {
  ...
  final param = ImType.wrap({'k1': 'v1', 'k2': 'v2'});

  final result = await isolate.compute(param);
} on UnsupportedImTypeWrappingException catch (e) {
  // Throws `UnsupportedImTypeWrappingException` when there is unsupported type.
}
```
